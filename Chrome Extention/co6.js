...
request.addEventListener('load', function (e) {
  // response comes back as "{data: [{attributes: {locations: [{...}], summary.title: '...'} ...}, {} ...]}"
  JSON.parse(this.responseText).data.forEach(function (el) {
    ...
    document.getElementById('pinterest').innerHTML = '<a href="http://pinterest.com/pin/create/button/?url=' + encodeURIComponent(el.links.self)  + '&media=' + imghost + el.attributes.multimedia[0]['@processed'].large.location +  '&description=' + el.attributes.summary.title + '" target="_blank"><img src="pinterest-5.svg" alt="Add to Pinterest"></a> ';
